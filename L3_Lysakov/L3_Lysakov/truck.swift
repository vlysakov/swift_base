...
struct Truck {
    ...
    @PropertyInterval(0...10000)
    private var volumeBody :Int {
        willSet {
            if newValue > 10000 {
                print ("В грузовой автомобиль нельзя загрузить более 10000 кг. Установлено максимальное значение 10000 кг.")
            } else if newValue < 0 {
    ...
    private mutating func loadingBody (_ action: DriveAction, _ weight: Int) {
        switch action {
        case .loadBody:
            if weight+useVolumeBody > 10000 {
                print ("Суммарная масса \(weight+useVolumeBody) больше вместимости багажника 10000 кг.")
            } else {
    ...
    func printf () {
        print ("Грузовой - модель \(model) год выпуска \(years) объем багажника \(volumeBody)")
